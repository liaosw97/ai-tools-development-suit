...
- [ ] 4.1 删除根目录下所有 `bash.exe.stackdump` 文件 [spec:repo-init#清理临时文件]
- [ ] 4.2 清理 `log/` 目录中的临时内容（保留目录结构或根据需要处理） [spec:repo-init#清理临时文件]
- [ ] 4.3 提交主仓库初始版本，验证 `git clone --recursive` 可正常工作 [spec:repo-init#初始化主仓库]

## Batch 5: 同步脚本

- [ ] 5.1 创建 `scripts/sync-upstream.sh`，实现主流程：遍历子项目 → fetch tags → 识别最新 release tag → checkout → 更新 versions.lock [spec:upstream-sync#同步所有子项目到最新 release tag]
- [ ] 5.2 实现 `--only <name>` 参数，支持仅同步指定子项目 [spec:upstream-sync#同步指定子项目]
- [ ] 5.3 实现同步前备份：将 `versions.lock` 复制为 `versions.lock.bak` [spec:upstream-sync#同步前自动备份当前版本]
- [ ] 5.4 实现测试失败回滚：测试命令返回非零时从 `versions.lock.bak` 读取旧 hash 并 checkout 回去 [spec:upstream-sync#同步后测试失败自动回滚]
- [ ] 5.5 实现无 tag 场景处理：输出警告并跳过该子项目 [spec:upstream-sync#子项目无可用 release tag]
- [ ] 5.6 实现远程不可达处理：`git fetch --tags` 失败时输出错误并跳过 [spec:upstream-sync#远程仓库不可达]
- [ ] 5.7 实现输出格式和退出码：汇总报告（成功/跳过/失败计数），退出码 0/1/2 [spec:upstream-sync#同步所有子项目到最新 release tag]
